...
def _responses_failed_event(adapter: "rev.ResponsesStreamAdapter", message: str) -> dict:
...
class ProxyV2Handler(BaseHTTPRequestHandler):
    ...
    def _forward(self, method: str):
        ...
        while True:
            ...
            if mode == PASSTHROUGH:
                ...
                try:
                    openai_body, fwd_ctx = fwd.anthropic_to_openai_request(
                ...
                try:
                    anthropic_body = rev.responses_to_anthropic_request(
            ...
            if mode == FORWARD:
                # chat 响应 → Anthropic
                if is_stream:
                    adapter = fwd.AnthropicStreamAdapter(fwd_ctx, target_model or "")
                    self._write_translated_stream(resp, adapter, fwd)
                else:
                    try:
                    ...
                    try:
                        ...
                        anthropic_resp = fwd.openai_to_anthropic_response(openai_resp, fwd_ctx)
                    except Exception as e:
            ...
            if is_stream:
                adapter = rev.ResponsesStreamAdapter(
                    model=target_model or "",
                    ctx={"tools": _tools_echo, "reasoning_effort": _r_effort})
                self._write_responses_stream(resp, adapter, rev)
            else:
                try:
                    raw_resp_body = resp.read()
                finally:
                ...
                try:
                    ...
                    responses_resp = rev.anthropic_to_responses_response(
    ...
    def _write_translated_stream(self, upstream_resp, adapter, mod) -> None:
        """正向流式（组合3）：上游 OpenAI chat SSE → Anthropic SSE。

        上游为 chat completions SSE：每事件一行 `data: {json}`，以 `data: [DONE]` 收尾。
        逐行解析 → adapter.feed(chunk) → 每个 Anthropic 事件经 mod.anthropic_sse_bytes 写出。
        """
        ...
        try:
            while not done:
                ...
                while b"\n" in buf:
                    ...
                    for ev in adapter.feed(chunk):
                        self._write_sse_chunk(mod.anthropic_sse_bytes(ev))
            ...
            try:
                ...
                self._write_sse_chunk(mod.anthropic_sse_bytes(err_event))
                self.wfile.write(b"0\r\n\r\n")
                self.wfile.flush()
    ...
    def _write_responses_stream(self, upstream_resp, adapter, mod) -> None:
        """反向流式（组合4）：上游网关 Anthropic SSE → Responses SSE。

        上游 Anthropic SSE 为 `event:xxx\\ndata:{json}`（冒号后无空格），事件间空行分隔。
        按 \\n\\n 切块，块内解析出 (event_type, data) → adapter.feed → mod.responses_sse_bytes 写出。
        ...
        try:
            while True:
                ...
                while b"\n\n" in buf:
                    ...
                    for ev in adapter.feed(ev_type, ev_data):
                        self._write_sse_chunk(mod.responses_sse_bytes(ev))
            ...
            try:
                self._write_sse_chunk(mod.responses_sse_bytes(
